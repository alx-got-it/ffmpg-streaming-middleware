...
def reduce_bitrate(bitrate: Bitrate, divide: int) -> Bitrate:
    ...
    video = (
        min_bitrate(int(bitrate.video_ / divide))
        if bitrate.video_ is not None and bitrate.video_ != 0
        else None
    )
    audio = (
        min_bitrate(int(bitrate.audio_ / divide))
        if bitrate.audio_ is not None and bitrate.audio_ != 0
        else None
    )
...
def cal_bitrate(bitrate, org_bitrate: Bitrate, index: int) -> Bitrate:
    return (
        ...
        else reduce_bitrate(org_bitrate, index)
    )
...
class AutoRep(object):
    def __init__(
        ...
        self.heights = (
            ...
            else [2160, 1440, 1080, 720, 480, 360, 240, 144]
        )
        self.bitrate = bitrate
        ...
        if heights is not None and bitrate is not None and len(heights) != len(bitrate):
            raise ValueError(
                "Длина списка высот должна быть равна длине списка битрейтов"
            )
    ...
    def __next__(self):
        ...
        return Representation(
            Size(width, height),
            cal_bitrate(self.bitrate, self.original_bitrate, self.index),
        )
