...
from .utiles import mkdir, get_path_info
...
class HLSMasterPlaylist:
    ...
    def _content(self) -> str:
        ...
        for rep in self.media.reps:
            if sub_exists := os.path.isfile(
                ...
                content += (
                    sub_info(rep, self.sub_path(rep)[0])
                    + stream_info(rep, sub_exists)
                    + self.stream_path(rep)
                )
            else:
    ...
    def _get_version(self) -> list:
        version = (
            "7" if self.media.options.get("hls_segment_type", "") == "fmp4" else "3"
        )
    ...
    def stream_path(self, rep):
        return [
            "{}_{}p.m3u8".format(
                os.path.basename(self.media.output_).split(".")[0], rep.size.height
            )
        ]
